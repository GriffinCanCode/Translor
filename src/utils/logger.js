...
const isNode = !isBrowser || !isRenderer;
...
// Create formatted log message (browser)
...
// Create browser logger implementation
const createBrowserLoggerImpl = () => {
  ...
  const sendToMain = (level, message, context) => {
    try {
      if (isBrowser && window.electronAPI?.log) {
...
// Create browser context logger implementation
const createBrowserContextLoggerImpl = (context = {}) => {
  const browserLogger = createBrowserLoggerImpl();
  return {
    error: (message, additionalContext = {}) => browserLogger.error(message, { ...context, ...additionalContext }),
    warn: (message, additionalContext = {}) => browserLogger.warn(message, { ...context, ...additionalContext }),
    info: (message, additionalContext = {}) => browserLogger.info(message, { ...context, ...additionalContext }),
    http: (message, additionalContext = {}) => browserLogger.http(message, { ...context, ...additionalContext }),
    debug: (message, additionalContext = {}) => browserLogger.debug(message, { ...context, ...additionalContext }),
    verbose: (message, additionalContext = {}) => browserLogger.verbose(message, { ...context, ...additionalContext })
  };
};
...
if (isNode) {
  // Import Node.js modules conditionally to prevent webpack bundling them for browser
  try {
    ...
    const { combine, timestamp, printf, colorize, label, json, errors } = format;
    ...
    const consoleFormat = combine(
      colorize({ all: true }),
      label({ label: 'Translor' }),
      timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }),
      printf(({ level, message, timestamp, label, ...metadata }) => {
        let metaStr = '';
    ...
    createContextLogger = (context = {}) => {
      return {
        error: (message, additionalContext = {}) => logger.error(message, { ...context, ...additionalContext }),
        ...
        http: (message, additionalContext = {}) => logger.http(message, { ...context, ...additionalContext }),
        debug: (message, additionalContext = {}) => logger.debug(message, { ...context, ...additionalContext }),
        verbose: (message, additionalContext = {}) => logger.verbose(message, { ...context, ...additionalContext })
      };
    };
    ...
    console.error('Error initializing Node.js logger:', error);
    // Fallback to browser logger if Node.js modules can't be loaded
    logger = createBrowserLoggerImpl();
    createContextLogger = createBrowserContextLoggerImpl();
    getLogPath = () => null;
  }
} 
